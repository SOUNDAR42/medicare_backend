...
import { api } from '../api';
import { User, Stethoscope, Building, Store, ArrowRight, Lock, Mail, MapPin, Phone } from 'lucide-react';

const Auth = () => {
    const navigate = useNavigate();
    const [role, setRole] = useState('patient'); // patient, doctor, hospital, pharmacy
    ...
    const RoleCard = ({ id, icon: Icon, label }) => (
        <button
            onClick={() => setRole(id)}
            className={`flex flex-col items-center justify-center p-6 rounded-2xl border-2 transition-all ${role === id
                ? 'border-primary bg-blue-50 text-primary shadow-md'
                : 'border-white bg-white hover:border-blue-100 text-gray-500 hover:shadow-sm'
                }`}
        >
            <Icon className={`h-8 w-8 mb-3 ${role === id ? 'text-primary' : 'text-gray-400'}`} />
            ...
            <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-8">
                    <div className="text-center lg:text-left">
                        ...
                        <p className="text-gray-600 text-lg">Choose your role to get started with the platform.</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex bg-gray-100 p-1 rounded-lg">
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${isLogin ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}
                            >
                                Login
                            </button>
                            <button
